...
new #[Title('New')] class extends Component {
    ...
    public function submitFiles()
    {
        if (!Auth::user()->subscribed() && count($this->uploadedFiles) > 1) {
            Flux::toast(heading: 'Upload Error', text: 'You must be a Basic plan subscriber to upload more than one file.', variant: 'danger', duration: 10000);
    ...
    <form wire:submit.prevent="submitFiles" class="mt-12">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-16">
            <div class="row-start-2 lg:col-span-2 lg:row-span-2">
                <flux:field>
                    @if (Auth::user()->subscribed())
                        <input hidden multiple type="file" accept="audio/*" id="fileInput"
                            @change="onFileInputChanged" />
                    @else
                        <input hidden type="file" accept="audio/*" id="fileInput" @change="onFileInputChanged" />
                    @endif

                    <flux:label for="fileInput" @dragover.prevent="handleDragOver" @dragleave.prevent="handleDragLeave"
                        @drop.prevent="onFileDropped">
                        <flux:card
                            class="text-center border-dashed border-2 p-12 lg:px-16 lg:py-28 w-full transition-colors">
                            <flux:text class="pointer-events-none"><span class="underline font-bold">Choose</span> or
                                drop audio files</flux:text>
                        </flux:card>
                    </flux:label>
                ...
                <div class="flex flex-row flex-wrap gap-2 items-end justify-between mt-10">
                    ...
                    <flux:button inset="top bottom" x-cloak x-show="localFiles.length > 0" size="sm" @click="clear"
                        label="Remove all files from upload queue" variant="ghost">
                        Clear All
                    </flux:button>
                </div>
                ...
                <ul class="mt-6 flex flex-col gap-2">
                    <template x-if="localFiles.length === 0">
                        <li>
                            <flux:callout inline>
                                <flux:callout.heading class="!opacity-50 self-center">0 files uploaded
                            ...
                            <li>
                                <flux:callout inline>
                                    ...
                                    <template x-if="file.error">
                                        <flux:callout.text><span x-text="file.errorMessage"></span> This file will NOT
                                            be uploaded.</flux:callout.text>
                                    </template>
                                    ...
                                    <x-slot name="controls" class="flex flex-row items-center gap-2">
                                        ...
                                        <flux:button @click="remove(index)"
                                            x-bind:disabled="!file.uploaded && !file.error" icon="x-mark"
                                            size="sm" label="Remove file from upload queue" variant="subtle">
                                        </flux:button>
                                    </x-slot>
            ...
            <flux:card class="flex flex-col gap-10">
                ...
                <flux:switch wire:model.live="completionEmail" label="Completion Email"
...
@script
    <script>
        Alpine.data('uploadHandler', () => ({
             ...
             */
            _initFileItem(file) {
                const allowedTypes = ['audio/wav', 'audio/x-wav', 'audio/mpeg', 'audio/mp3', 'audio/mp4',
                ...
                return {
                    index: this.localFiles.length - 1,
                    error: !isAudioFile || !isValidSize
                };
            },
